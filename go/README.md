# EuroRates Golang SDK



The Golang SDK for the EuroRates API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.
...
## Install
```bash
go get github.com/voxgig-sdk/euro-rates-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/euro-rates-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/euro-rates-sdk/go=../euro-rates-sdk/go
```
...
```go
func NewEuroRatesSDK(options map[string]any) *EuroRatesSDK
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *EuroRatesSDK
```

Creates a test-mode client with mock transport. Both arguments may be `nil`.

### EuroRatesSDK methods
...
All entities implement the `EuroRatesEntity` interface.
...
```go
exchangeRate, err := client.ExchangeRate(nil).Load(map[string]any{"from": "from", "to": "to"}, nil)
if err != nil {
    panic(err)
}
...
```
github.com/voxgig-sdk/euro-rates-sdk/go/
├── euro-rates.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/euro-rates-sdk/go`) re-exports everything needed
