...
import (
	...
	omni "github.com/voxgig-sdk/euro-rates-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.EuroRatesError); is {
				out := map[string]any{
					"name":    "EuroRatesError",
...
func omniCtx(arg any, client *sdk.EuroRatesSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.EuroRatesSDK); is {
			client = live
