...
local EuroRatesSDK = {}
EuroRatesSDK.__index = EuroRatesSDK
...
EuroRatesSDK._make_feature = _make_feature


function EuroRatesSDK.new(options)
  local self = setmetatable({}, EuroRatesSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function EuroRatesSDK:options_map()
...
function EuroRatesSDK:get_utility()
  return Utility.copy(self._utility)
end


function EuroRatesSDK:get_root_ctx()
  return self._rootctx
end


function EuroRatesSDK:prepare(fetchargs)
...
function EuroRatesSDK:direct(fetchargs)
...
-- by passing it.
function EuroRatesSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function EuroRatesSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "EuroRatesSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function EuroRatesSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = EuroRatesSDK.new(sdkopts)
  sdk.mode = "test"
