...
class EuroRatesTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            EuroRatesTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = EuroRatesTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = EuroRatesTestRunner.getenv("EURO_RATES_TEST_LIVE")
        override = EuroRatesTestRunner.getenv("EURO_RATES_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = EuroRatesTestRunner.getenv(key)
        ...
        explain = EuroRatesTestRunner.getenv("EURO_RATES_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["EURO_RATES_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if EuroRatesTestRunner._test_control is not None:
            return EuroRatesTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                EuroRatesTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            EuroRatesTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = EuroRatesTestRunner.load_test_control()
...
def load_env_local():
    EuroRatesTestRunner.load_env_local()


def env_override(m):
    return EuroRatesTestRunner.env_override(m)


def entity_data(v):
    return EuroRatesTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return EuroRatesTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return EuroRatesTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return EuroRatesTestRunner.load_test_control()


def live_client_options():
    return EuroRatesTestRunner.live_client_options()


def live_delay_ms():
    return EuroRatesTestRunner.live_delay_ms()
