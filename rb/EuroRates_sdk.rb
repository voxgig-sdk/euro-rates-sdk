...
require_relative 'EuroRates_types'
...
class EuroRatesSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = EuroRatesHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = EuroRatesHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, EuroRatesFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    EuroRatesUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = EuroRatesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = EuroRatesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = EuroRatesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = EuroRatesHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = EuroRatesError.new(
        "graphql_error", "EuroRatesSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
