# EuroRates SDK ExchangeRate entity
...
class ExchangeRateEntity
  ...
  def make
    opts = @_entopts.dup
    ExchangeRateEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = EuroRatesHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [ExchangeRate, Hash] the current ExchangeRate data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = EuroRatesHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of ExchangeRate fields)
  def match_get
  ...
  #
  # @param reqmatch [ExchangeRateLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.ExchangeRate.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [ExchangeRate, Hash] the loaded ExchangeRate; raises EuroRatesError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = EuroRatesHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
