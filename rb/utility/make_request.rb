...
module EuroRatesUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = EuroRatesResponse.new({})
    ...
    if fetch_err
      ...
      response = EuroRatesResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = EuroRatesResponse.new(fetched)
    else
