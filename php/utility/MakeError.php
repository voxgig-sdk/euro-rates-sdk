...
class EuroRatesMakeError
{
    public static function call(?EuroRatesContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new EuroRatesContext([], null);
        }
        $op = $ctx->op ?? new EuroRatesOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new EuroRatesResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof EuroRatesError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new EuroRatesError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof EuroRatesError) {
