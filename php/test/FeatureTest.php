...
class FtCtrl extends EuroRatesControl
{
...
class FtClient
{
    ...
    private EuroRatesUtility $utility;

    public function __construct(EuroRatesUtility $utility, array $options)
    {
    ...
    public function get_utility(): EuroRatesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (EuroRatesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public EuroRatesContext $rootctx;
    ...
    public function __construct(
        ...
        $utility = new EuroRatesUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (EuroRatesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = EuroRatesConfig::make_feature($name);
    ...
    private static function buildUrl(EuroRatesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof EuroRatesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new EuroRatesResponse($fetched);
        }
    ...
    private function fail(EuroRatesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (EuroRatesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
