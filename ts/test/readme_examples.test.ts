...
//     (\`new EuroRatesSDK(...)\` / \`EuroRatesSDK.test(...)\` become
//     \`EuroRatesSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"currency":{"test01":{"id":"test01"}},"exchange_rate":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new EuroRatesSDK(...)\` and any
// \`EuroRatesSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+EuroRatesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/EuroRatesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
