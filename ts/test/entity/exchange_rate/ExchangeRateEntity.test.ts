...
describe('ExchangeRateEntity', async () => {
  ...
  // `test.live.delayMs`; only sleeps when EURORATES_TEST_LIVE=TRUE.
  afterEach(liveDelay('EURORATES_TEST_LIVE'))
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'exchange_rate.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set EURO_RATES_TEST_EXCHANGE_RATE_ENTID JSON to run live')
      return
    }
    ...
    const exchange_rate_ref01_data_dt0 = await exchange_rate_ref01_ent.load(exchange_rate_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['EURO_RATES_TEST_EXCHANGE_RATE_ENTID']
