...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.EURORATES_TEST_LIVE ||
    'TRUE' === process.env.EURORATES_TEST_OVERRIDE
  ) {
  ...
  m.EURORATES_TEST_EXPLAIN = process.env.EURORATES_TEST_EXPLAIN || m.EURORATES_TEST_EXPLAIN
