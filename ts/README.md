# EuroRates TypeScript SDK



The TypeScript SDK for the EuroRates API — a type-safe, entity-oriented client with full async/await support.
...
| Method | Returns | Description |
| --- | --- | --- |
| `EuroRatesSDK.test(testopts?, sdkopts?)` | `EuroRatesSDK` | Create a test-mode client. |
...
```ts
const exchange_rate = await client.ExchangeRate().load({ from: 'from', to: 'to' })
```
...
euro-rates/
├── src/
│   ├── EuroRatesSDK.ts        # Main SDK class
