# EuroRates TypeScript SDK Reference

Complete API reference for the EuroRates TypeScript SDK.
...
```ts
new EuroRatesSDK(options?: object)
```
...
#### `EuroRatesSDK.test(testopts?, sdkopts?)`
...
**Returns:** `EuroRatesSDK` instance in test mode.
...
Return the parent `EuroRatesSDK` instance.
...
```ts
const result = await client.ExchangeRate().load({ from: 'from', to: 'to' })
```
