...
class EuroRatesRetryFeature extends EuroRatesBaseFeature
{
    ...
    public function init(EuroRatesContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (EuroRatesContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(EuroRatesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(EuroRatesContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
