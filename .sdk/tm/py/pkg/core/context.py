...
from projectname_sdk.core.control import EuroRatesControl
...
class EuroRatesContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, EuroRatesSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, EuroRatesResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, EuroRatesResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return EuroRatesError(code, msg, self)
